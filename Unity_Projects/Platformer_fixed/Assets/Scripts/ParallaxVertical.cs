...
public class ParallaxVertical : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if(parallaxFraction.y > 0f && temp.y > startPosition.y + size.y) startPosition.y += size.y;
        else if(parallaxFraction.y > 0f && temp.y < startPosition.y - size.y)  startPosition.y -= size.y;
        transform.position = new Vector3(startPosition.x + offset.x, startPosition.y + offset.y, transform.position.z);
    }
}
